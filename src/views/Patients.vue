<template>
  <div>
    ...
    <v-dialog v-model="dialogVaccineAppointment" persistent max-width="600">
      <v-card>
        <v-card-title class="headline">
          ...
          <!-- {{ txt.SCHEDULE_VACCINE }} for {{ patient.firstname }} {{ patient.lastname }} -->
    ...
    <v-data-table
      ...
      :headers="headers"
      :items="patients"
      class="elevation-4 mt-5 patients-table-container"
      :search="search"
    ...
    >
      ...
      <template v-slot:[`item.actions`]="{ item }">
        ...
        <span class="appointment-info" v-if="checkForAppointment(item)"
          >{{ appointmentDate(item)
          }}<v-icon small right>mdi-needle</v-icon></span
        >
      </template>
    </v-data-table>
...
export default {
  ...
  methods: {
    ...
    deleteItemConfirm() {
      let allAppointments = [...this.appointments, ...this.confirmedAppointments];
      let appointmentsToDelete = allAppointments.filter(
        (app) => app.patientId === this.patient.id
      );
